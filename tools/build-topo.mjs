...
for (const p of PAGES) {
  ...
  // One dotted route that climbs the dominant hill to the summit pin and crests
  // down the far side into the distance — uniform weight/opacity the whole way.
  // Tuned to index.html's focal (1040, 360); retune the control points if that
  // bump moves. Lives in #topoLayer so it parallax-drifts with the contours (and
  // the pin, which mirrors the drift).
  let trail = '';
  if (p.peak) {
    trail =
      `\n      <path d="M180,908 C330,884 384,800 520,772 C690,738 660,612 800,560 C928,514 946,428 1036,362` +
      ` C1128,326 1200,300 1274,258 C1352,214 1402,250 1472,214 C1522,188 1556,196 1588,184"` +
      ` stroke="#B85C3C" stroke-width="2.3" stroke-dasharray="0.4 7" opacity="0.5"></path>`;
  }
